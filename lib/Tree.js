class Tree {
  ...
  _cloneChildren(newValue, oldValue) {
    this._traverse((item) => {
      if (newValue === item.value) {
        item.children.push(...this._displayLeafs(oldValue));
      }
    });
  }

  _editNode(newValue, oldValue, parentValue) {
    this._addNode(newValue, parentValue);
    this._cloneChildren(newValue, oldValue);
    this._removeNode(oldValue);
  }
}
